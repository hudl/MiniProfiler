...
where not exists (select 1 from MiniProfilers where Id = @Id)"; // this syntax works on both mssql and sqlite
        ...
        protected virtual void SaveTiming(DbConnection connection, MiniProfiler profiler, Timing timing)
        {
                ...
                new
                    {
                        ...
                        MongoTimingsDurationMilliseconds = timing.MongoTimingsDurationMilliseconds,
                        ...
                        HasMongoTimings = timing.HasMongoTimings,
                        HasDuplicateMongoTimings = timing.HasDuplicateMongoTimings,
                        ExecutedReaders = timing.ExecutedReaders,
                        ExecutedScalars = timing.ExecutedScalars,
                        ExecutedNonQueries = timing.ExecutedNonQueries,
                        ExecutedMongoReaders = timing.ExecutedMongoReaders,
                        ExecutedMongoScalars = timing.ExecutedMongoScalars,
                        ExecutedMongoNonQueries = timing.ExecutedMongoNonQueries
                    });
            ...
            if (timing.HasMongoTimings)
            {
                foreach (var st in timing.MongoTimings)
                {
                    SaveMongoTiming(connection, profiler, st);
                }
            }
...
values      (@Id,
        ...
        private MiniProfiler LoadIndividually(DbConnection connection, object keyParameter)
        {
            ...
            if (result != null)
            {
				...
				var mongoTimings = LoadFor<MongoTiming>(connection, keyParameter);
        ...
        public const string TableCreationScript =
				...
				create table MiniProfilerMongoTimings
  				(
     				RowId                          integer not null identity constraint PK_MiniProfilerMongoTimings primary key clustered,
     				Id                             uniqueidentifier not null,
     				MiniProfilerId                 uniqueidentifier not null,
     				ParentTimingId                 uniqueidentifier not null,
     				ExecuteType                    tinyint not null,
                ...
                create nonclustered index IX_MiniProfilerMongoTimings_MiniProfilerId on MiniProfilerMongoTimings (MiniProfilerId)
