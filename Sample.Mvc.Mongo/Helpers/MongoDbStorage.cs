...
namespace StackExchange.Profiling.Storage
{
    ...
    public class MongoDbStorage : DatabaseStorageBase
    {
        ...
        private MongoCollection<MongoTimingPoco> _mongotimings;
        private MongoCollection<MongoTimingPoco> MongoTimings
        {
            get
            {
                if (_mongotimings == null)
                {
                    _mongotimings = Db.GetCollection<MongoTimingPoco>("mongotimings");
                }
                return _mongotimings;
            }
        }
        ...
        private List<MongoTiming> LoadMongoTimings(Guid profilerId)
        {
            var mongoTimings = new List<MongoTiming>();

            var query = Query.EQ("MiniProfilerId", profilerId.ToString());

            var mongoTimingPocos = MongoTimings.Find(query).ToList();

            foreach (var poco in mongoTimingPocos)
            {
                mongoTimings.Add(new MongoTiming
                {
                    Id = Guid.Parse(poco.Id),
                    ParentTimingId = Guid.Parse(poco.ParentTimingId),
                    ExecuteType = poco.ExecuteType,
